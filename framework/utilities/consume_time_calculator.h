...
public:
    ...
    inline void startCalculate(int epoch) { (void)epoch; }
    inline void endCalculate(int epoch) { (void)epoch; }
    inline double getAverageResult(int endEpoch, const std::string& header) { (void)endEpoch; (void)header; return -1;}
    [[nodiscard]] inline int getCurrentEpoch() const { return -1; }
    void increaseEpoch() {}
    void setEpoch(epoch_size_t epoch) { (void)epoch; }
