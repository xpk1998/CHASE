    ...
    async fn seal<'a>(
        ...
        let mut batch = tokio::task::spawn_blocking(move || {
            batch
                ...
                .into_par_iter()
                .for_each(|tx| {
                    // Already JSON-encoded TypedTransaction (CHASE / SmallBank path).
                    if serde_json::from_slice::<TypedTransaction>(&tx).is_ok() {
                        return;
                    }

                    let rlp = Rlp::new(&tx);
                    if let Ok((rlp_decoded_tx, _)) = TypedTransaction::decode_signed(&rlp) {
                        let se = serde_json::to_vec(&rlp_decoded_tx).unwrap();
                        tx.clear();
                        tx.extend(se);
                    }
                    // Non-EVM payloads (e.g. legacy Narwhal string tests) pass through unchanged.
                });
            batch
        })
        .await
