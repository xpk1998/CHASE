...
impl PrimaryNodeInner {
        ...
        State: ExecutionState + Send + Sync + 'static,
    {
        ...
        // Set CHASE_USE_TUSK=1 to use Tusk (async DAG-BFT) instead of Bullshark.
        let ordering_engine = if std::env::var("CHASE_USE_TUSK").is_ok() {
            info!("Starting Narwhal with Tusk consensus (Chase stack)");
            ConsensusProtocol::Tusk(Tusk::new(
                committee.clone(),
                store.consensus_store.clone(),
